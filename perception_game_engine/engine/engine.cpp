...
void engine_execute()
{
    ...
    while (window_instance->process_messages())
    {
        ...
        std::cout << "fps " << g_vars.engine.render_thread.frame_data.current_hz << "\n";
    ...
    while (true)
    {
        g_vars.system.tick_count++;
		g_vars.set_update_end_tickcount(thread_ids_t::thread_id_system, g_vars.system.tick_count);
        p_sleep(1);
    }
